...
void
timestamp2date(unsigned int t, char*fmt_date, char* out_buf, int buf_len) {
...
timestamp_today() {
	...
	return (unsigned int)today;
}

unsigned int
timestamp_yesterday() {
	unsigned int today = timestamp_today();
	unsigned int yesterday = today - 24 * 60 * 60;
	return (unsigned int)yesterday;
}
